...
BASE_URL = 'https://www.netflix.com/api/msl/cadmium/'
...
class MslClient(object):
    ...
    def load_manifest(self, viewable_id):
        ...
        try:
            ...
            if manifest.get('result'):
                ...
                self.msl_session['playbackContextId'] = manifest['result']['viewables'][0]['playbackContextId']
                self.msl_session['drmContextId'] = manifest['result']['viewables'][0]['drmContextId']
                self.msl_session['cert'] = manifest['result']['viewables'][0]['cert']
                self.msl_session['pssh'] = manifest['result']['viewables'][0]['psshb64']
                return manifest
    ...
    def get_license(self, challenge):
        ...
        if not self.msl_session['playbackContextId']:
            raise LicenseError(
        ...
        license_request_data = {
            ...
            'uiVersion': 'akira',
            'languages': self.msl_session["languages"],
            'playbackContextId': self.msl_session["playbackContextId"],
            'drmContextIds': [self.msl_session['drmContextId']],
            'challenges': [{
    ...
    def __decrypt_msl_payload(self, payload):
        ...
        for chunk in payload_chunks:
            ...
            payload = payloadchunk['payload']
            decoded_payload = base64.b64decode(payload).decode('utf-8')
            encryption_envelope = json.loads(decoded_payload)
            cipher = AES.new(
                self.msl_session['session_keys']['encryption_key'],
                AES.MODE_CBC,
                base64.b64decode(encryption_envelope['iv'])
            )

            plaintext = cipher.decrypt(base64.b64decode(encryption_envelope['ciphertext']))
            ...
            if plaintext.get("compressionalgo") == "GZIP":
                ...
                data = zlib.decompress(decode_data, 16 + zlib.MAX_WBITS)
            else:
                data = base64.b64decode(data)
            #data = base64.b64decode(data).decode('utf8')

            chunks.append(data.decode("utf8"))

        decrypted_payload = json.loads(''.join(chunks))[1]["payload"]["data"]
        decrypted_payload = base64.b64decode(decrypted_payload)

        return json.loads(decrypted_payload)
    def __compress_data_sd(self, data):
        out = io.BytesIO()
    ...
    def __generate_msl_request_data(self, data):
        ...
        header_envelope = pymsl.utils.msl_encrypt(self.msl_session, pymsl.utils.dumps(header))

        header_signature = HMAC.new(self.msl_session['session_keys']['sign_key'], header_envelope, SHA256).digest()
        ...
        compressed_data = self.__compress_data_sd(serialized_data)
        payload = {
            'messageid': self.msl_session['message_id'],
            'data': compressed_data.decode("utf8"),
